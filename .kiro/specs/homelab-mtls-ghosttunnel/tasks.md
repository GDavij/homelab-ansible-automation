...
- [x] 4. Add excalidraw service to PKI configuration
  - Add excalidraw to pki_services configuration (using service name, not proxy name)
  - Configure DNS name (draw.lab) and IP address as SANs
  - _Requirements: 2.1, 2.2, 2.3_

- [x] 5. Add memos service to PKI configuration
...
- [x] 6. Add maybe-finance service to PKI configuration
...
- [x] 7. Generate application certificate files
  - Generate certificate files for each application's GhostTunnel sidecar (excalidraw.crt, memos.crt, maybe-finance.crt)
...
- [x] 13. Configure Upsnap GhostTunnel host network proxy
...
- [x] 14. Add certificate volume mounts for Upsnap
...
- [x] 15. Configure firewall rules for Upsnap host network service
...
- [x] 16. Configure Upsnap with auto-generated machine configuration
...
- [ ] 34. Update Traefik static configuration for host network services
...
- [ ] 35. Configure SSL passthrough for GhostTunnel services
...
- [ ] 44.3. Implement pre-deployment validation checks
  - Source virtual environment: `source .venv/bin/activate`
  - Verify required directories exist: `/opt/core/upsnap`, `/opt/apps/memos`, `/opt/apps/maybe`
...
- [ ] 44.4. Create deployment logging and monitoring
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 44.5. Implement idempotency verification
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 44.6. Create service dependency verification
  - Source virtual environment: `source .venv/bin/activate`
  ...
  - Test DNS resolution for all service domains (snap.lab, draw.lab, daily.lab, finance.lab)
  - _Requirements: Professional execution_

- [ ] 44.7. Implement security validation checks
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 44.8. Create performance baseline measurements
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 44.9. Implement health check automation
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 44.10. Create documentation and runbooks
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 45. Test PKI certificate generation and deployment
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 46. Test Upsnap infrastructure service deployment
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 47. Test Upsnap mTLS connectivity and WoL functionality
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 48. Test Traefik static configuration for host network routing
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 49. Test application services deployment (Excalidraw, Memos, Maybe Finance)
  - Source virtual environment: `source .venv/bin/activate`
  - Deploy application services: `ansible-playbook site.yml --tags "apps_gateways" --limit bastion_automation`
  - Verify all application containers are running: `docker ps | grep -E "(excalidraw|memos|maybe-finance)"`
  - Check sidecar GhostTunnel containers: `docker ps | grep sidecar`
  - Verify network_mode configuration: `docker inspect excalidraw-sidecar | grep NetworkMode`
  - Test shared network stack: `docker exec excalidraw netstat -tlnp | grep 8443`
  - _Requirements: 5.1, 5.2, 5.3, 6.1, 6.2, 6.3, 7.1, 7.2, 7.3_

- [ ] 50. Test application mTLS connectivity
  - Source virtual environment: `source .venv/bin/activate`
  - Test Excalidraw mTLS: `curl --cert client.crt --key client.key --cacert ca.crt https://draw.lab:8443`
  ...
  - Verify certificate validation failures: `curl -k https://draw.lab:8443` (should fail)
  - Check application accessibility only via localhost from sidecar
  - _Requirements: 1.3, 1.4, 1.5_

- [ ] 51. Test Traefik service discovery for Docker network services
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 52. Test certificate authority validation and client authentication
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 53. Test container restart and certificate deployment
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 54. Test firewall integration and security
  - Source virtual environment: `source .venv/bin/activate`
...
- [ ] 55. Test end-to-end functionality and integration
  - Source virtual environment: `source .venv/bin/activate`
  - Test complete workflow: Client → Traefik → GhostTunnel → Application
  - Verify all services are accessible via their DNS names (snap.lab, draw.lab, daily.lab, finance.lab)
...
- [ ] 56. Final checkpoint - Ensure all tests pass
